...
def problem4(number_of_stairs, step_size, starting_point, window):
    ...
    endingpoint=rg.Point(starting_point.x+(number_of_stairs*step_size),starting_point.y-(number_of_stairs*step_size))
    endingpoint.attach_to(window)
    starting_point.attach_to(window)

    for k in range (number_of_stairs):
        line=rg.Line(rg.Point(starting_point.x+(k*step_size),starting_point.y-(k*step_size)),rg.Point(starting_point.x+(k*step_size),starting_point.y-(k*step_size)-step_size))
        line.color='magenta'
        line.thickness=3
        line.attach_to(window)

        line = rg.Line(rg.Point(starting_point.x + (k * step_size), starting_point.y - (k * step_size)-step_size),rg.Point(starting_point.x + (k * step_size)+step_size, starting_point.y - (k * step_size) - step_size))
        line.color = 'black'
        line.thickness = 3
        line.attach_to(window)
